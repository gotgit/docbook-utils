...
<head>
	...
	<xsl:element name="link">
		<xsl:attribute name="rel">stylesheet</xsl:attribute>
		<xsl:attribute name="href">
			<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_css"/>treestyles.css</xsl:attribute>
		<xsl:attribute name="type">text/css</xsl:attribute>
	</xsl:element>
	<xsl:element name="script">
		<xsl:attribute name="type">text/javascript</xsl:attribute>
		<xsl:attribute name="src">
			<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_js"/>marktree.js</xsl:attribute>&#160;
	</xsl:element>
</head>
...
<body>
	...
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	  <xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_js"/>header_utf.js</xsl:attribute>
	</xsl:element>
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	...
	<!-- place Flash -->
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	<xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_flash"/>flashobject.js</xsl:attribute>&#160;
	</xsl:element>
	...
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>&#160;
		var fo = new FlashObject("<xsl:value-of select="$document_root" />/includes/mmflash/visorFreemind.swf", "visorFreeMind", "100%", "500", 6, "#9999ff");
		fo.addParam("quality", "high");
		fo.addParam("bgcolor", "#ffffff");
		fo.addVariable("openUrl", "_blank");
		fo.addVariable("initLoadFile", "<xsl:value-of select="$freemind_src" />");
	...
	<!-- place footer -->
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	  <xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_js"/>footer_utf.js</xsl:attribute>
	</xsl:element>
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
