...
<xsl:template name="webpage.sponsor">
    <xsl:apply-templates select="$autolayout/autolayout/headlink" mode="webpage.sponsor" />
</xsl:template>

<xsl:template match="headlink" mode="webpage.sponsor">
  <xsl:if test="@rel='sponsor'">
    <div class="sponsor"> <a>
      <xsl:attribute name="href">
        <xsl:value-of select="@href" />
      </xsl:attribute>
