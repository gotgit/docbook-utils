...
<xsl:template name="process.image">
  ...
  <xsl:variable name="filename">
   <xsl:choose>
     <xsl:when test="starts-with($filename2, '..') or starts-with($filename2, '/')" >
  ...
  <xsl:variable name="filename.for.graphicsize2">
    <xsl:choose>
  ...
  <xsl:variable name="filename.for.graphicsize">
   <xsl:choose>
     <xsl:when test="starts-with($filename.for.graphicsize2, '..') or starts-with($filename.for.graphicsize2, '/')" >
       <xsl:call-template name="filename-basename">
         <xsl:with-param name="filename" select="$filename.for.graphicsize2"/>
       </xsl:call-template>
     </xsl:when>
     <xsl:otherwise>
       <xsl:value-of select="$filename.for.graphicsize2"/>
     </xsl:otherwise>
   </xsl:choose>
  </xsl:variable>
